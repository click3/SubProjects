...
class AdobeFlashPlayerData(FeedUpdateData):
    ...
    def setBody(self, body):
        ...
        p = re.compile('<strong>Windows</strong>.*?<td.*?</td>.*?<td>(.*?)</td>', re.DOTALL)
        result = p.search(body)
        if (result == None):
            self.__updateExist = False
            return
        version = result.group(1)
        title = 'AdobeFlashPlayer' + version
...
def main():
    FeedUpdate(__file__, 'http://www.adobe.com/jp/software/flash/about/').run()
